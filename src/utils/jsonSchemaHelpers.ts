...
export const deeplyRemoveNestedProperty: (schema: JsonSchema, path: string[]) => JsonSchema =
    (schema, path) => {
      if (!schema.properties)
        throw new Error(`Schema has no properties`)
      if (path.length === 0) {
      ...
      if (path.length === 1) {
        return {
          ...schema,
          properties: (Object.fromEntries(Object.entries(schema.properties).filter(
              // @ts-ignore
              ([key]) => key !== path[0])
    ...
    (schema, path, newField) => {
      ...
      if (path.length === 1) {
        ...
        return {
          ...schema,
          properties: {
            ...(Object.fromEntries(Object.entries(schema.properties).filter(
                // @ts-ignore
                ([key]) => key !== path[0])
            ) as JsonSchema['properties']),
            [newField]: value
